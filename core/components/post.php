...
function cl_post_data($post = array()) {
	...
	$post["owner"]         = array(
		...
		'cont_monetization' => $post_owner_data['cont_monetization'],
		'subscription_price' => $post_owner_data['subscription_price'],
		'is_online' => $post_owner_data["is_online"]
	);
	...
	if ($post["owner"]["cont_monetization"] == "Y") {
		$post["content_view"] = false;
		...
		if (not_empty($post["is_owner"])) {
			...
			if (not_empty($cl["is_logged"])) {
				if (cl_is_subscribed($cl["me"]["id"], $post["user_id"]) == true) {
					$post["content_view"] = true;
				}
			}
		}
