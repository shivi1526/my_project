...
function cl_icon($icon_name = "") {
    ...
    $path1 = cl_strf("themes/%s/statics/fd_icons/%s.svg", $config['theme'], $icon_name);
    $path2 = cl_strf("themes/%s/statics/md_icons/%s.svg", $config['theme'], $icon_name);

    if (file_exists(cl_full_path($path1))) {
        return file_get_contents(cl_full_path($path1));
    }
    else if (file_exists(cl_full_path($path2))) {
        return file_get_contents(cl_full_path($path2));
    }
    else{
        return "<b>?</b>";
    }
...
function cl_get_restricted_usernames() {
    global $config;

    $data = array();
    $usernames = explode(",", $config["username_restrictions"]);

    if (is_array($usernames)) {
        foreach($usernames as $uname) {
            array_push($data, trim($uname));
        }
    }
...
function cl_get_restricted_useremails() {
    global $config;

    $data = array();
    $useremails = explode(",", $config["useremail_restrictions"]);

    if (is_array($useremails)) {
        foreach($useremails as $email) {
            array_push($data, trim($email));
        }
    }
...
function cl_get_media($media = '', $is_upload = false){
    ...
    elseif ($config['ws3_storage'] == 'on') {
        ...
        $media_url  = cl_strf("https://s3.%s.wasabisys.com/%s/%s", $ws3_reg, $ws3_bucket, $media);
        return $media_url;
    }
    else {
...
function cl_cropimg($max_width, $max_height, $source_file, $dst_dir, $quality = 80) {
    ...
    if ($width_new > $width) {
        $h_point = (int)((abs($height) - abs($height_new)) / 2);
        @imagecopyresampled($dst_img, $src_img, 0, 0, 0, $h_point, $max_width, $max_height, $width, $height_new);
    } 

    else {
        $w_point = (int)((abs($width) - abs($width_new)) / 2);
...
function cl_gen_path($data = array()) {
    $upload_dirs = array(
        ...
        cl_full_path(cl_strf("upload/receipts/%s/%s",date('Y'), date('m'))),
        ...
        cl_full_path(cl_strf("upload/documents/%s/%s",date('Y'), date('m')))
    );
...
function cl_upload2wasabi_s3($filename = null, $del_localfile = "Y") {
    ...
    if ($cl['config']['ws3_storage'] == 'off') {
        ...
        if (empty($cl['config']['ws3_api_key'])) {
            return false;
        }

        else if(empty($cl['config']['ws3_api_secret_key'])) {
            return false;
        }
            ...
            try {
                ...
                $amazon_s3        = new \Aws\S3\S3Client(array(
                    'version'     => 'latest',
                    'endpoint' => 'https://s3.'.$cl['config']['ws3_bucket_region'].'.wasabisys.com',
                ...
                $up_aws_object     = $amazon_s3->putObject(array(
                    'Bucket'       => $cl['config']['ws3_bucket_name'],
                    'Key'          => $filename,
                    'Body'         => $file_res,
                    'ACL'          => 'public-read',
                    'CacheControl' => 'max-age=3153600'
                ...
                if ($del_localfile == "Y") {
                    if ($amazon_s3->doesObjectExist($cl['config']['ws3_bucket_name'], $filename)) {
                        cl_delete_loc_media($filename);
                    }
                }
    ...
    if ($cl['config']['ws3_storage'] == 'off') {
        ...
        if (empty($cl['config']['ws3_api_key'])) {
            ...
            try {
                ...
                if ($amazon_s3->doesObjectExist($cl['config']['ws3_bucket_name'], $filename) != true) {
                    return true;
                }
    ...
    if (not_empty($path) && file_exists(cl_full_path($path))) {
        try {
            ...
            $placeholders = array(
                ...
                'upload/default/admin-avatar.png',
...
function cl_time2str($ptime = 0) {
    ...
    foreach ($a as $secs => $str) {
        ...
        if ($diff >= 1) {
            $diff = round($diff);
            return cl_strf("%s %s %s",$diff, (($diff > 1) ? cl_translate($a_plural[$str]) : cl_translate($str)), cl_translate("ago"));
        }
    }
}

function cl_date($format = "", $date = 0) {
    global $cl;

    $months = array(
        "January" => cl_translate("January"),
        "February" => cl_translate("February"),
        "March" => cl_translate("March"),
        "April" => cl_translate("April"),
        "May" => cl_translate("May"),
        "June" => cl_translate("June"),
        "July" => cl_translate("July"),
        "August" => cl_translate("August"),
        "September" => cl_translate("September"),
        "October" => cl_translate("October"),
        "November" => cl_translate("November"),
        "December" => cl_translate("December"),
    ...
    foreach($months as $k => $v) {
        if (preg_match("/{$k}/i", $date)) {
            $date = str_ireplace($k, $months[$k], $date); break;
        }
    }
...
function cl_get_custom_code($type = false) {
    global $cl;

    if ($type == "headerjs") {
        $code = file_get_contents(cl_full_path(cl_strf("themes/%s/statics/custom_code/header.js", $cl["config"]["theme"])));

        return $code;
    }

    elseif ($type == "footerjs") {
        $code = file_get_contents(cl_full_path(cl_strf("themes/%s/statics/custom_code/footer.js", $cl["config"]["theme"])));

        return $code;
    }

    elseif ($type == "headercss") {
...
function cl_get_censored_words_array() {
    global $db;

    $data = array();

    $censored_words_list = file_get_contents(cl_full_path("core/loc_db/censor_words.csv"));

    if ($censored_words_list) {
        $censored_words_list = explode(",", $censored_words_list);

        if (is_array($censored_words_list)) {

            $censored_words_list = array_filter($censored_words_list, function($value) {
                return $value !== '';
            });
...
function cl_percentage_of($percentage, $of) { 
    $percent = $percentage / $of; 
    
    return  number_format( $percent * 100, 0); 
}

function cl_detect_unsafe_image($file_path = "") {
    global $cl;
    ...
    $req_data = array(
       "requests" => array(
            array(
                "image" => array(
                   "source" => array("imageUri" => cl_get_media($file_path)) 
                ), 
                "features" => array(
                    array("type" => "SAFE_SEARCH_DETECTION", "maxResults" => 1), 
                    array("type" => "WEB_DETECTION", "maxResults" => 2) 
                ) 
            )
        )
    );
    ...
    try {
        ...
        $req_data = json($req_data, true);

        $ch = curl_init();

        curl_setopt($ch, CURLOPT_URL, cl_strf("https://vision.googleapis.com/v1/images:annotate?key=%s", $cl["config"]["google_ai_vision_key"]));

        curl_setopt($ch, CURLOPT_HTTPHEADER, array(
            "Content-Type: application/json",
            cl_strf("Content-Length: %s", strlen($req_data))
        ));
        ...
        if (not_empty($data["error"])) {
            return false;
        }

        if (not_empty($data["responses"][0]["error"])) {
            return false;
        }

        elseif (in_array($data["responses"][0]["safeSearchAnnotation"]["adult"], array("LIKELY", "VERY_LIKELY"))) {
            return true;
        }

        elseif (in_array($data["responses"][0]["safeSearchAnnotation"]["racy"], array("LIKELY", "VERY_LIKELY"))) {
            return true;
        }
...
function cl_create_user_avatar($text = "", $destination_url = false) {
    global $cl;

    $font_size = 140;
    $width = 600;
    $height = 600;
    $font = cl_full_path(cl_strf("themes/%s/statics/sys_fonts/arial.ttf", $cl["config"]["theme"]));
    $bg_color = array(0, 94, 169);
    $text_color = array(255, 255, 255);
    $text = cl_croptxt($text, 3);

    $image = @imagecreate($width, $height) or die("Cannot Initialize new GD image stream");

    imagecolorallocate($image, $bg_color[0], $bg_color[1], $bg_color[2]);

    $font_color = imagecolorallocate($image, $text_color[0], $text_color[1], $text_color[2]);

    $text_bounding_box = imagettfbbox($font_size, 0, $font, $text);

    $y = abs(ceil(($height - $text_bounding_box[5]) / 2));
    $x = abs(ceil(($width - $text_bounding_box[2]) / 2));
